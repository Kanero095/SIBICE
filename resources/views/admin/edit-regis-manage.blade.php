<x-templates>
    <title>Edit Registration Verify | SIBICIE'</title>

    <div class="overflow-auto">
        <div class="py-5 px-3">
            <div class="pb-5">
                <a href="{{ route('registBinaanUKM-Verify') }}"
                    class="button w-full bg-red-600 px-3 py-1 rounded-lg text-white font-bold drop-shadow hover:text-black">
                    Batal
                </a>
            </div>
                    ...
                    @foreach ($verify as $data)
                        <tr class="border-b-2 border-black">
                            ...
                            <td class="text-center px-2">
                                ...
                                @endif
                                <form action="{{ route('updateRegist', $data->Slug) }}" class="px-3 grid grid-rows-1"
                                    method="POST">
                                    @csrf
                                    <select class="w-full px-1 rounded" name="statusTenant" id="statusTenant">
                                        <option>Select</option>
                                        <option value="Diterima">Diterima</option>
                                        <option value="Ditolak">Ditolak</option>
                                    </select>
                                    <button type="submit"
                                        class="bg-lime-600 px-3 py-1 mt-1 rounded-lg text-white font-bold drop-shadow hover:text-black">
                                        Submit
                                    </button>
                                </form>
                            </td>
